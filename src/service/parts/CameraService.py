...
class CameraService:
        ...
        try:
            ...
            for fno in sorted(self.options.camera_motion.cameras.keys()):
                ...
                (
                    ...
                    rep_inner_global_poses,
                    ratio,
                    (nearest_data_set_idx, nearest_bone_name),
                    (left_data_set_idx, left_bone_name),
                    (right_data_set_idx, right_bone_name),
                    (top_data_set_idx, top_bone_name),
    ...
    def execute_rep_camera(
        ...
        cf: VmdCameraFrame,
        past_cf: VmdCameraFrame,
        org_inner_global_poses: dict,
        org_inner_square_poses: dict,
        rep_inner_global_poses: dict,
        ratio: float,
        nearest_data_set_idx: int,
        nearest_bone_name: str,
        left_data_set_idx: int,
        left_bone_name: str,
        right_data_set_idx: int,
        right_bone_name: str,
        top_data_set_idx: int,
        top_bone_name: str,
        bottom_data_set_idx: int,
        bottom_bone_name: str,
        camera_length: float,
    ):
        ...
        if camera_length < 5:
            ...
            pos_ratio = (
                ...
                if length_ratio == ratio
                    ...
                    self.camera_options[nearest_data_set_idx].body_ratio,
                    max(1 / self.camera_options[nearest_data_set_idx].body_ratio, ratio),
                )
            )
        ...
        if (
            ...
            rep_inner_square_poses[(bottom_data_set_idx, bottom_bone_name)] = self.calc_project_square_vec(
            ...
            length_unit = (
                (
                    (
                        self.camera_options[top_data_set_idx].body_ratio
                        + self.camera_options[bottom_data_set_idx].body_ratio
                    )
                    / 2
                )
                / 5
        ...
        logger.info(
            ...
            nearest_bone_name.replace("実体", ""),
            (top_data_set_idx + 1),
            top_bone_name.replace("実体", ""),
            (bottom_data_set_idx + 1),
            bottom_bone_name.replace("実体", ""),
            round(offset_length, 5),
            offset_angle,
        )
    ...
    def calc_camera_ratio(self, fno: int, cf: VmdCameraFrame):
        ...
        org_face_inner_global_poses, org_face_inner_square_poses = self.calc_inner_index(
            fno, org_inner_global_poses, org_inner_square_poses, HEAD_BONE_NAMES, 0, 0.1
        )
        # 画面内に映っている体幹系INDEX
        org_trunk_inner_global_poses, org_trunk_inner_square_poses = self.calc_inner_index(
            fno, org_inner_global_poses, org_inner_square_poses, TRUNK_BONE_NAMES + LEG_BOTTOM_BONE_NAMES, 0, 0.1
        )
        ...
        if len(org_face_inner_square_poses.keys()) > 0:
            ...
            if len(org_trunk_inner_square_poses.keys()) > 0:
                ...
                # innerに追加
                org_inner_global_poses[(nearest_data_set_idx, nearest_bone_name)] = all_org_global_poses[
                    (nearest_data_set_idx, nearest_bone_name)
                ]
                org_inner_square_poses[(nearest_data_set_idx, nearest_bone_name)] = all_org_project_square_poses[
                    (nearest_data_set_idx, nearest_bone_name)
                ]
        ...
        logger.debug(
            "f: %s, nearest d: %s, k: %s, v: %s, s: %s",
            fno,
            nearest_data_set_idx,
            nearest_bone_name,
            all_org_global_poses[(nearest_data_set_idx, nearest_bone_name)],
            all_org_project_square_poses[(nearest_data_set_idx, nearest_bone_name)],
        )

        # 画面端ボーンを計算する
        (top_data_set_idx, top_bone_name), (bottom_data_set_idx, bottom_bone_name) = self.calc_top_botom_index(
            fno, cf, org_inner_square_poses, org_inner_global_poses, 0, 0.1
        )
        (left_data_set_idx, left_bone_name), (right_data_set_idx, right_bone_name) = self.calc_left_right_index(
            fno, cf, org_inner_square_poses, org_inner_global_poses, 0, 0.1
        )
        ...
        if left_data_set_idx >= 0 and left_bone_name:
            logger.debug(
                "f: %s, left d: %s, k: %s, v: %s, s: %s",
                fno,
                left_data_set_idx,
                left_bone_name,
                org_inner_global_poses[(left_data_set_idx, left_bone_name)],
                all_org_project_square_poses[(left_data_set_idx, left_bone_name)],
            )

        if right_data_set_idx >= 0 and right_bone_name:
            logger.debug(
                "f: %s, right d: %s, k: %s, v: %s, s: %s",
                fno,
                right_data_set_idx,
                right_bone_name,
                org_inner_global_poses[(right_data_set_idx, right_bone_name)],
                all_org_project_square_poses[(right_data_set_idx, right_bone_name)],
            )

        if top_data_set_idx >= 0 and top_bone_name:
            logger.debug(
                "f: %s, top d: %s, k: %s, v: %s, s: %s",
                fno,
                top_data_set_idx,
                top_bone_name,
                org_inner_global_poses[(top_data_set_idx, top_bone_name)],
                all_org_project_square_poses[(top_data_set_idx, top_bone_name)],
            )

            org_top_diff = MVector3D(org_inner_global_poses[(top_data_set_idx, top_bone_name)]).distanceToPoint(
                MVector3D(org_inner_global_poses[(nearest_data_set_idx, nearest_bone_name)])
            )
            rep_top_diff = MVector3D(rep_inner_global_poses[(top_data_set_idx, top_bone_name)]).distanceToPoint(
                MVector3D(rep_inner_global_poses[(nearest_data_set_idx, nearest_bone_name)])
            )

        if bottom_data_set_idx >= 0 and bottom_bone_name:
            logger.debug(
                "f: %s, bottom d: %s, k: %s, v: %s, s: %s",
                fno,
                bottom_data_set_idx,
                bottom_bone_name,
                org_inner_global_poses[(bottom_data_set_idx, bottom_bone_name)],
                all_org_project_square_poses[(bottom_data_set_idx, bottom_bone_name)],
            )

            org_bottom_diff = MVector3D(
                org_inner_global_poses[(bottom_data_set_idx, bottom_bone_name)]
            ).distanceToPoint(MVector3D(org_inner_global_poses[(nearest_data_set_idx, nearest_bone_name)]))
            rep_bottom_diff = MVector3D(
                rep_inner_global_poses[(bottom_data_set_idx, bottom_bone_name)]
            ).distanceToPoint(MVector3D(rep_inner_global_poses[(nearest_data_set_idx, nearest_bone_name)]))
        ...
        logger.debug(
            "f: %s, ratio: %s, nearest d: %s, k: %s, v: %s, s: %s",
            fno,
            ratio,
            nearest_data_set_idx,
            nearest_bone_name,
    ...
    def calc_inner_index(
        self,
        fno: int,
        all_global_poses: dict,
        all_square_poses: dict,
        bone_name_list: list,
        x_offset: float,
        y_offset: float,
    ):
        ...
        if not bone_name_list:
            ...
            if len(square_poses) > 0:
                # 画面内に映っている対象となるINDEX
                refrected_indexes = np.where(
                    ((0 - x_offset) <= square_poses[:, 0])
                    & (square_poses[:, 0] <= (1 + x_offset))
                    & ((0 - y_offset) <= square_poses[:, 1])
                    & (square_poses[:, 1] <= (1 + y_offset))
                    & (0 < square_poses[:, 2])
                    & (square_poses[:, 2] < 1)
                )
            ...
            for (data_set_idx, bone_name), square_pos in all_square_poses.items():
                if (
                    bone_name in bone_name_list
                    and (0 - x_offset) <= square_pos[0] <= (1 + x_offset)
                    and (0 - y_offset) <= square_pos[1] <= (1 + y_offset)
                    and 0 < square_pos[2] < 1
                ):
    ...
    def calc_top_botom_index(
        ...
        for yi in y_indexes:
            if square_poses[yi][1] > square_poses[refrected_indexes[0][y_indexes[0]]][1] + 0.1:
                ...
                logger.debug(
                    "f: %s, 画面上端 break: yi: %s, top: %s", fno, yi, square_poses[refrected_indexes[0][y_indexes[0]]][1]
                )
                break

            logger.debug(
                "f: %s, 画面 check: square_poses[yi][1]: %s, square_poses[yi][2]: %s, top_edge_pos[1]: %s, top_edge_pos[2]: %s",
                fno,
                square_poses[yi][1],
                square_poses[yi][2],
                top_edge_pos[1],
        ...
        for yi in reversed(y_indexes):
            if square_poses[yi][1] < square_poses[refrected_indexes[0][y_indexes[-1]]][1] - 0.2:
                ...
                logger.debug(
                    "f: %s, 画面下端 break: yi: %s, top: %s", fno, yi, square_poses[refrected_indexes[0][y_indexes[-1]]][1]
                )
                break
    ...
    def calc_org_project_square_poses(
        self,
        fno: int,
        cf: VmdCameraFrame,
        start_idx: int,
        ...
        for data_set_idx, camera_option in self.camera_options.items():
            for link_idx, org_link in enumerate(camera_option.org_links[start_idx:end_idx]):
                ...
                org_global_3ds = MServiceUtils.calc_global_pos(
                    data_set.camera_org_model, org_link, data_set.org_motion, fno
                )
                for bone_name, org_vec in org_global_3ds.items():
                    if (
                        bone_name in camera_option.org_link_target.keys()
                        and (data_set_idx, bone_name) not in all_org_project_square_poses
                    ):
                        # 処理対象ボーンである場合、データを保持
                        all_org_global_poses[(data_set_idx, bone_name)] = org_vec.data()
                        all_org_project_square_poses[(data_set_idx, bone_name)] = self.calc_project_square_vec(
                            cf, org_vec
                        ).data()

        [
            logger.test("f: %s, k: %s, v: %s, s: %s", fno, k, v, sv)
            for (k, v), (sk, sv) in zip(all_org_global_poses.items(), all_org_project_square_poses.items())
        ]
    ...
    def calc_camera_qq(self, cf: VmdCameraFrame):
        # カメラ角度
        camera_qq = MQuaternion.fromEulerAngles(
            -math.degrees(cf.euler.x()), math.degrees(cf.euler.y()), math.degrees(cf.euler.z())
        )
    ...
    def prepare(self, data_set_idx: int):
        ...
        # 体幹系はループ外で定義する
        self.prepare_link(
            data_set.camera_org_model,
            data_set.rep_model,
            data_set.camera_offset_y,
            org_links,
            org_link_target,
            rep_links,
            ["頭頂実体", "頭"],
            ["頭頂実体", "頭", "首", "首根元", "上半身2", "上半身"],
        )
        self.prepare_link(
            data_set.camera_org_model,
            data_set.rep_model,
        ...
        for direction in ["左", "右"]:
            self.prepare_link(
                ...
                org_link_target,
                rep_links,
                ["{0}手首".format(direction), "{0}手首".format(direction)],
            ...
            self.prepare_link(
                ...
                org_link_target,
                rep_links,
                ["{0}つま先実体".format(direction), "{0}足ＩＫ".format(direction)],
            ...
            self.prepare_link(
                ...
                org_link_target,
                rep_links,
                ["{0}足".format(direction), "下半身"],
                ["{0}足".format(direction)],
            )

        self.camera_options[data_set_idx] = CameraOption(
            org_links,
            org_link_target,
            rep_links,
            org_total_height,
    ...
    def prepare_link(
        ...
        org_links: list,
        org_link_target: dict,
        rep_links: dict,
        link_bone_name_list: list,
        target_bone_name_list: list,
    ):
                ...
                if target_bone_name in org_link.all().keys() and target_bone_name in rep_model.bones:
                    ...
                    if "頭頂実体" == target_bone_name:
                        rep_link.get("頭頂実体").position.setY(
                            float(rep_link.get("頭頂実体").position.y()) + float(camera_offset_y)
                        )
...
class CameraOption:
    def __init__(
        ...
        org_link_target: dict,
        rep_links: dict,
        org_total_height: float,
        org_face_length: float,
        org_heads: float,
        rep_total_height: float,
        rep_face_length: float,
        rep_heads: float,
        body_ratio: float,
        head_ratio: float,
    ):
