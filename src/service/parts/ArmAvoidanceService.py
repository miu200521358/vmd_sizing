    ...
    def execute_avoidance_pool(self, data_set_idx: int, direction: str):
        try:
            ...
            while len(fnos) > 0:
                ...
                prev_total_rep_collision_vec = self.execute_avoidance_frame(data_set_idx, direction, self.avoidance_options[(data_set_idx, direction)], fno, prev_fno, prev_total_rep_collision_vec)
            ...
            for bone_name in ["{0}腕".format(direction), "{0}ひじ".format(direction)]:
                data_set.motion.smooth_filter_bf(0, bone_name, data_set.rep_model.bones[bone_name].getRotatable(), \
                                                 data_set.rep_model.bones[bone_name].getTranslatable(), \
                                                 config={"freq": 30, "mincutoff": 0.03, "beta": 0.01, "dcutoff": 0.25})
    ...
    def execute_avoidance_frame(self, data_set_idx: int, direction: str, avoidance_options: ArmAvoidanceOption, fno: int, prev_fno: int, prev_total_rep_collision_vec: dict):
        ...
        for ((avoidance_name, avodance_link), avoidance) in zip(avoidance_options.avoidance_links.items(), avoidance_options.avoidances.values()):
            ...
            for arm_link in avoidance_options.arm_links:
                ...
                collision, near_collision, rep_collision_vec = obb.judge_collision(rep_global_3ds[arm_link.last_name()])
                ...
                if near_collision:
                    logger.info("－近接あり: f: %s(%s-%s:%s), 元: %s", fno, \
        ...
        return total_rep_collision_vec
