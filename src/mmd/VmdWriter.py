...
class VmdWriter():
    ...
    def write(self):
        ...
        bone_frames = self.data_set.motion_vmd_data.get_bone_frames()
        morph_frames = self.data_set.motion_vmd_data.get_morph_frames()
            
        if len(bone_frames) > 0 or len(morph_frames) > 0:
            try:
                # モデル名を20byteで切る
                model_bname = self.data_set.rep_model_data.name.encode('cp932').decode('shift_jis').encode('shift_jis')[:20]
            except Exception:
                logger.warning("モデル名に日本語・英語で判読できない文字が含まれているため、仮モデル名を設定します。 %s", self.data_set.rep_model_data.name, decoration=MLogger.DECORATION_BOX)
        ...
        for cf in self.data_set.motion_vmd_data.lights:
            cf.write(fout)
        fout.write(struct.pack('<L', len(self.data_set.motion_vmd_data.shadows)))  # セルフ影キーフレーム数
        for cf in self.data_set.motion_vmd_data.shadows:
            cf.write(fout)
        fout.write(struct.pack('<L', len(self.data_set.motion_vmd_data.showiks)))  # モデル表示・IK on/offキーフレーム数
        for sf in self.data_set.motion_vmd_data.showiks:
            sf.write(fout)
