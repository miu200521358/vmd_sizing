...
cdef class MQuaternion():
    ...
    cpdef MVector3D toEulerAngles(self):
        ...
        cdef DTYPE_FLOAT_t pitch = cmath.asin(max(-1, min(1, -2.0 * (yz - xw))))
        cdef DTYPE_FLOAT_t yaw = 0
        cdef DTYPE_FLOAT_t roll = 0
        
        if pitch < (cmath.pi / 2):
            if pitch > -(cmath.pi / 2):
                yaw = cmath.atan2(2.0 * (xz + yw), 1.0 - 2.0 * (xx + yy))
                roll = cmath.atan2(2.0 * (xy + zw), 1.0 - 2.0 * (xx + zz))
            else:
                # not a unique solution
                roll = 0.0
                yaw = -cmath.atan2(-2.0 * (xy - zw), 1.0 - 2.0 * (yy + zz))
        else:
            # not a unique solution
            roll = 0.0
            yaw = cmath.atan2(-2.0 * (xy - zw), 1.0 - 2.0 * (yy + zz))
    ...
    cpdef DTYPE_FLOAT_t calcTheata(self, MQuaternion v):
        ...
        cdef DTYPE_FLOAT_t sinOfAngle = cmath.sin(theta)
        return sinOfAngle
    ...
    cpdef MQuaternion c_fromAxisAndAngle(self, MVector3D vec3, DTYPE_FLOAT_t angle):
        ...
        cdef DTYPE_FLOAT_t length = cmath.sqrt(x * x + y * y + z * z)
        ...
        cdef DTYPE_FLOAT_t s = cmath.sin(a)
        cdef DTYPE_FLOAT_t c = cmath.cos(a)
    ...
    cpdef MQuaternion c_fromAxisAndQuaternion(self, MVector3D vec3, MQuaternion qq):
        ...
        cdef DTYPE_FLOAT_t a = cmath.acos(min(1, max(-1, qq.scalar())))
        cdef DTYPE_FLOAT_t s = cmath.sin(a)
        cdef DTYPE_FLOAT_t c = cmath.cos(a)
    ...
    cpdef MQuaternion c_fromDirection(self, MVector3D direction, MVector3D up):
        ...
        cdef MVector3D xAxis = MVector3D.crossProduct(up, zAxis)
        ...
        xAxis.normalize()
        cdef MVector3D yAxis = MVector3D.crossProduct(zAxis, xAxis)
    ...
    cpdef MQuaternion c_rotationTo(self, MVector3D fromv, MVector3D tov):
        ...
        cdef DTYPE_FLOAT_t d = MVector3D.dotProduct(v0, v1) + 1.0
        cdef MVector3D axis
    ...
    cpdef MQuaternion c_fromEulerAngles(self, DTYPE_FLOAT_t pitch, DTYPE_FLOAT_t yaw, DTYPE_FLOAT_t roll):
        ...
        cdef DTYPE_FLOAT_t c1 = cmath.cos(yaw)
        cdef DTYPE_FLOAT_t s1 = cmath.sin(yaw)
        cdef DTYPE_FLOAT_t c2 = cmath.cos(roll)
        cdef DTYPE_FLOAT_t s2 = cmath.sin(roll)
        cdef DTYPE_FLOAT_t c3 = cmath.cos(pitch)
        cdef DTYPE_FLOAT_t s3 = cmath.sin(pitch)
    ...
    cpdef MQuaternion c_nlerp(self, MQuaternion q1, MQuaternion q2, DTYPE_FLOAT_t t):
        ...
        cdef DTYPE_FLOAT_t dot = MQuaternion.dotProduct(q1, q2)
        if dot < 0.0:
            q2b = -q2b
    ...
    cpdef MQuaternion c_slerp(self, MQuaternion q1, MQuaternion q2, DTYPE_FLOAT_t t):
        ...
        if (1.0 - dot) > 0.0000001:
            angle = cmath.acos(max(0, min(1, dot)))
            sinOfAngle = cmath.sin(angle)
            if sinOfAngle > 0.0000001:
                factor1 = cmath.sin((1.0 - t) * angle) / sinOfAngle
                factor2 = cmath.sin(t * angle) / sinOfAngle
...
cdef class MMatrix4x4():
    ...
    cpdef lookAt(self, MVector3D eye, MVector3D center, MVector3D up):
        ...
        forward.normalize()
        cdef MVector3D side = MVector3D.crossProduct(forward, up).normalized()
        cdef MVector3D upVector = MVector3D.crossProduct(side, forward)
    ...
    cpdef perspective(self, DTYPE_FLOAT_t verticalAngle, DTYPE_FLOAT_t aspectRatio, DTYPE_FLOAT_t nearPlane, DTYPE_FLOAT_t farPlane):
        ...
        cdef DTYPE_FLOAT_t sine = cmath.sin(radians)

        if sine == 0:
            return
        
        cdef DTYPE_FLOAT_t cotan = cmath.cos(radians) / sine
