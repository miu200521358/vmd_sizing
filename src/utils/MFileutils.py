...
def read_history(mydir_path):
    # ファイル履歴
    file_hitories = {"vmd": [], "org_pmx": [], "rep_pmx": [], "camera_vmd": [], "camera_pmx": [], "smooth_vmd": [], "smooth_pmx": [], "parent_vmd": [], "parent_pmx": [], "max": 50}
    ...
    try:
        with open(os.path.join(mydir_path, 'history.json'), 'r') as f:
            ...
            # キーが揃っているかチェック
            for key in ["vmd", "org_pmx", "rep_pmx", "camera_vmd", "camera_pmx", "smooth_pmx", "smooth_vmd", "parent_vmd", "parent_pmx"]:
...
# 全親Y回転VMD出力ファイルパス生成
# base_file_path: モーション全親Y回転VMDパス
# pmx_path: 変換先モデルPMXパス
...
def get_output_parent_vmd_path(base_file_path: str, pmx_path: str, output_parent_vmd_path: str, is_force=False):
    # モーション全親Y回転VMDパスの拡張子リスト
    if not os.path.exists(base_file_path) or not os.path.exists(pmx_path):
        return ""

    # モーション全親Y回転VMDディレクトリパス
    motion_parent_vmd_dir_path = get_dir_path(base_file_path)
    # モーション全親Y回転VMDファイル名・拡張子
