                        ...
                        if is_floor_hand:
                            if bf_idx != 0 and is_prev_next_enable_key("センター", motion.frames, bf_idx) and is_prev_next_enable_key("上半身", motion.frames, bf_idx):
                                ...
                                if (org_wrist_y <= org_palm_length * 1.5 or org_reverse_wrist_y <= org_palm_length * 1.5):
                                    ...
                                    motion.frames["センター"][bf_idx].position.setY(motion.frames["センター"][bf_idx].position.y() + rep_center_diff + wrist_thickness["左"])
                                    motion.frames["センター"][bf_idx].key = True

                                    logger.info("hand_floor wrist: center: %s", motion.frames["センター"][bf_idx].position)

                                    print("○手首床近接あり: f: %s, センター補正: %s(%s)" % (bf.frame, rep_center_diff, wrist_thickness["左"]))
                                ...
                                if (org_wrist_y <= org_palm_length * 1.5 or org_reverse_wrist_y <= org_palm_length * 1.5) and (org_upper_y > org_leg_y * 1.2 or org_upper_y > org_reverse_leg_y * 1.2 or org_leg_y > org_palm_length * 2 or org_reverse_leg_y > org_palm_length * 2 ):
                                    ...
                                    if org_wrist_y <= org_palm_length * 1.5  and org_reverse_wrist_y <= org_palm_length * 1.5:
                                        ...
                                        # # 目標となる手首の中間
                                        # target_rep_wrist_middle_pos = (rep_wrist_pos + rep_reverse_wrist_pos) / 2
                                        # # 目標Y
                                        # org_target_y = min(org_wrist_y, org_reverse_wrist_y) * arm_palm_diff_length
                                        # org_thickness_y = abs(wrist_thickness[org_direction])
                                        # target_rep_wrist_middle_pos.setY( org_target_y + org_thickness_y)
                                        # logger.info("hand_floor org_target_y: %s, org_thickness_y: %s", org_target_y, org_thickness_y)
                                        # logger.info("hand_floor target_rep_wrist_middle_pos: %s", target_rep_wrist_middle_pos)
